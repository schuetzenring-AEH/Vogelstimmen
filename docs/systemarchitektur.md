...
│  │                       │        │  VOUT → 5V nur U2 │                    │
│  └───────────┬───────────┘        └────────┬───────────┘                    │
│              │ 12V_SW                      │ 5V (nicht zum Zähler)          │
│              │ → B3, B5, B8                │                                │
│              │                             │                                │
│   ┌──────────▼──────┐          ┌───────────▼──────────────┐                 │
│   │ B8: LED-Versorg.│          │ B4: DY-SV17F Audio-Modul │                 │
│   │                 │          │                          │                 │
│   │ 12V_SW → LED+   │          │  VCC ← 5V (nur U1/B3)   │                 │
│   │ GND → LED-       │          │  IO1–IO8 ← Taster       │                 │
│   └─────────────────┘          │  BUSY → B2 (HOLD/Release)│                 │
│                                │  SPK+/SPK- → B7         │                 │
│   ┌─────────────────┐          │  USB ← extern (Wartung) │                 │
│   │ B6: Taster-IF   │          └──────────────────────────┘                 │
│   │                 │                                                      │
...
| 12V_PROT | nach P-FET + TVS | ~BAT+ minus 0,1V | immer | B2 Latch (kein UVLO-Teiler, E-UVLO-01) |
| 12V_SW | nach Latch P-FET | ~12V_PROT minus 0,1V | nur bei Wiedergabe | B3 Buck VIN, **B5 Hengstler/One-Shot**, B8 LEDs |
| 5V | Buck-Ausgang | 5,0 V ± 2% | nur bei Wiedergabe | B4 DY-SV17F |
...
|---|-------------|------------|----------------------|
| D1 | DY-SV17F statt WT588D | 8 IO-Trigger, USB Drag&Drop, Amp integriert, verfügbar | WT588D (abgekündigt), Adafruit FX (35mA idle) — **E-AUDIO-01** |
| D2 | Hardware-Latch statt Sleep | 0 µA Ruhestrom, kein Firmware nötig | Software-Sleep (DY-SV17F hat keinen im IO-Modus) — **E-LATCH-01** |
| D3 | Mode 0 (Flanke) statt Mode 1 (Pegel) | Sound spielt nach kurzem Druck komplett durch | Mode 1 (Taster müsste 10s gehalten werden) — **E-AUDIO-02** |
| D4 | Kein RC-Verzögerungsglied | Kurzer Druck (<30ms) = kein Sound = akzeptabel | Soft-Start, ATtiny85, 74HC125-Buffer — **E-AUDIO-01** |
| D5 | 5V nur für Audio-Modul | DY-SV17F; Zähler an 12V_SW + One-Shot | Dual-Rail 3,3V + 5V — **E-BUCK-01** |
| D6 | Zähler zählt Sessions, nicht Einzeldrücke | BUSY pulsiert nicht bei Retrigger, Verhalten akzeptiert | Separate Zähl-Logik pro Taster — **E-CNT-03** |
| D7 | Alle LEDs gemeinsam an 12V_SW | 0 Extra-Bauteile, zeigt "Gerät aktiv" | Einzel-LED pro Taster (16 Bauteile extra) — **E-LED-01** |
| D8 | Latch schaltet 12V (nicht Buck-EN) | True Zero Standby, LEDs direkt über 12V_SW | Buck-EN schalten (LEDs bräuchten separate 12V) — **E-LATCH-01** |
| D9 | **Kein** UVLO-Teiler / kein Supervisor-IC | Rev 1: SGM809 verbrannt; Idle ~12 µA; leere Batterie = Box stirbt von selbst | UVLO-Teiler am Buck-EN; SGM809 — **E-UVLO-01** (ersetzt die alte D9-Formulierung) |
...
| S6 | ~~UVLO-Spannungsteiler 10,5V~~ → bewusst entfallen **E-UVLO-01** | erledigt |
